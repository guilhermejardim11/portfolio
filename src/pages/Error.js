	...
	switch (error.status) {
		case 401:
			...
			message = (
				...
				>
					You need an authorization type <b>{{ error }}</b> to access
					this page. Sad for you, of course.
				</Trans>
			);
			break;
		...
		case 403:
			...
			message = (
				<Trans
					...
					values={{ error: error.status }}
				>
					This place is forbidden, <b>{{ error }}</b>. Don't even try
					it.
				</Trans>
			);
			break;
		...
		case 503:
			...
			message = (
				<Trans
					...
					values={{ error: error.status }}
				>
					And that's all the information you're gonna get. Now move,{' '}
					<b>{{ error }}</b>...
				</Trans>
			);
			break;
		...
		case 408:
			...
			message = (
				<Trans
					...
					values={{ error: error.status }}
				>
					Why would you stop my request prematurely, ah? Wait... You
					clicked the link '<b>{{ error }}</b>' times??? HOW?
				</Trans>
			);
			break;
		...
		case 404:
			...
			message = (
				...
				>
					...
					trying to hack my <b>{{ error }}</b>th system.
				</Trans>
			);
			break;
		...
		case 500:
			...
			message = (
				<Trans
					...
					values={{ error: error.status }}
				>
					Our server is, ah... taking a <b>{{ error }}</b> hour break!
					Yea, sure, why not...
				</Trans>
			);
			break;
